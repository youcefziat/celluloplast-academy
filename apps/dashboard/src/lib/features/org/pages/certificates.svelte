<script lang="ts">
  ...
  import CertificateEditorHeader from '$features/course/components/ceritficate/editor/certificate-editor-header.svelte';
  import TemplatesPanel from '$features/course/components/ceritficate/editor/panels/templates-panel.svelte';
  import ContentPanel from '$features/course/components/ceritficate/editor/panels/content-panel.svelte';
  import ColorsPanel from '$features/course/components/ceritficate/editor/panels/colors-panel.svelte';
  import {
    certificateEditorStore,
    type CertificateEditorPanel
  } from '$features/course/components/ceritficate/editor/store/certificate-editor.store.svelte';
  ...
  const sampleRenderData = $derived({
    ...
    courseName: $t('course.navItem.certificates.editor.sample_course'),
    courseDescription: store.draft.descriptionOverride || $t('course.navItem.certificates.editor.sample_description'),
...
{#if $currentOrg.id}
  <div class="ui:bg-background ui:text-foreground flex h-dvh flex-col">
    <CertificateEditorHeader
      title={$t('celluloplast_org_certificates.editor_title')}
      {backHref}
        ...
        <div class="ui:border-border border-b px-5 py-4">
          {#if store.activePanel === 'templates'}
            ...
            <p class="ui:text-muted-foreground mt-1 text-xs">
              {$t('celluloplast_org_certificates.panel_templates_subtitle')}
            </p>
